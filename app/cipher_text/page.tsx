...
export default function AdminNexus() {
  ...
  const [darkMode, setDarkMode] = useState(false); // Alabaster Default
  ...
  const mergedData = useMemo(() => {
    ...
    const legacy = (googleFormData as any[] || []).filter(i => i !== null).map((item, idx) => {
      ...
      return {
        ...
        proof: item["Link to your social media verification post:"], picture: formatDriveUrl(item["Professional picture"]),
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={`text-center p-12 md:p-16 rounded-[4rem] border backdrop-blur-3xl shadow-2xl transition-all ${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-black/5'}`}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); if(password === "cipher02040") setIsAuthenticated(true); else alert("Invalid Cipher"); }} className="relative group">
          ...
          <button 
            ...
            className={`absolute right-4 top-1/2 -translate-y-1/2 transition-colors ${darkMode ? 'text-white/20 hover:text-smartan-orange' : 'text-black/20 hover:text-[#08263f]'}`}
          >
        ...
        <div className={`w-[450px] border-r flex flex-col transition-all ${darkMode ? 'bg-black/20 border-white/10' : 'bg-black/[0.02] border-black/5'}`}>
          <div className={`p-6 border-b transition-all ${darkMode ? 'border-white/10' : 'border-black/5'}`}>
            <div className="relative">
              <HiSearch className={`absolute left-4 top-1/2 -translate-y-1/2 opacity-20`} />
            ...
            {selectedEntity ? (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} key={selectedEntity.id} className="max-w-4xl mx-auto">
                ...
                <div className="flex items-start justify-between mb-12">
                   <div className="flex gap-10">
                      <div className={`w-40 h-40 rounded-[3rem] border overflow-hidden shadow-2xl flex-shrink-0 transition-all ${darkMode ? 'bg-[#08263f] border-white/10' : 'bg-white border-black/5'}`}>
                        {selectedEntity.picture ? <img src={selectedEntity.picture} className="w-full h-full object-cover" crossOrigin="anonymous" referrerPolicy="no-referrer" /> : <HiUser className="text-black/10 w-full h-full p-10" />}
                      </div>
                      <div>
                ...
                <div className={`grid grid-cols-2 gap-8 p-12 rounded-[4rem] border mb-10 transition-all ${darkMode ? 'bg-white/2 border-white/5' : 'bg-black/5 border-black/5'}`}>
                   <div className="space-y-1"><span className="text-[10px] font-black text-smartan-teal uppercase tracking-widest block opacity-40">Location</span><p className="text-xl font-bold uppercase italic tracking-tighter">{selectedEntity.location}</p></div>
                   <div className="space-y-1"><span className="text-[10px] font-black text-smartan-teal uppercase tracking-widest block opacity-40">RSVP Mode</span><p className="text-xl font-bold uppercase italic tracking-tighter text-smartan-orange">{selectedEntity.mode}</p></div>
                   <div className="space-y-1"><span className="text-[10px] font-black text-smartan-teal uppercase tracking-widest block opacity-40">Contact Number</span><p className="text-xl font-bold italic tracking-tighter">{selectedEntity.phone}</p></div>
                   <div className="space-y-1"><span className="text-[10px] font-black text-smartan-teal uppercase tracking-widest block opacity-40">Email Hash</span><p className="text-sm font-mono opacity-60">{selectedEntity.email}</p></div>
                   
                   <div className="col-span-2 pt-10 border-t border-current/5 mt-4">
                      <span className="text-[10px] font-black text-smartan-pink uppercase tracking-widest block mb-4 opacity-40">Deep Intelligence</span>
      ...
      <AnimatePresence>
        {confirmingAction && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/95 backdrop-blur-xl p-8">
            <div className={`p-16 rounded-[5rem] max-w-xl w-full text-center shadow-2xl border transition-all ${darkMode ? 'bg-[#08263f]/30 border-white/10' : 'bg-white border-black/5'}`}>
              <HiExclamationCircle className="text-smartan-orange text-[100px] mx-auto mb-10" />
              <h2 className="text-4xl font-display font-black uppercase italic mb-6 tracking-tighter">Confirm Signal</h2>
              <p className="opacity-40 mb-12 uppercase text-[10px] tracking-[0.5em] leading-loose">Executing {confirmingAction.type} for <br/><span className={`${darkMode ? 'text-white' : 'text-[#08263f]'} font-bold text-xl tracking-normal`}>{confirmingAction.name}</span></p>
              <div className="flex gap-6">
                <button onClick={() => setConfirmingAction(null)} className={`flex-1 py-7 rounded-[2rem] font-black uppercase text-[11px] tracking-widest border transition-all ${darkMode ? 'bg-white/5 border-white/10 text-white' : 'bg-black/5 border-black/10 text-[#08263f]'}`}>Abort</button>
                <button onClick={() => { setActiveTimer({ id: confirmingAction.id, type: confirmingAction.type, count: 5 }); setConfirmingAction(null); }} className="flex-1 py-7 rounded-[2rem] bg-smartan-orange text-[#050B14] font-black uppercase text-[11px] tracking-widest shadow-2xl shadow-smartan-orange/40">Execute</button>
              </div>
            </div>
          </motion.div>
        )}
